...
OUT = r'C:\Users\Administrator\d1_big_batches'
...
for table in ['code_task_log', 'table_d19']:
    ...
    # Get existing count
    cur2 = conn.cursor()
    cur2.execute(f'SELECT COUNT(*) as cnt FROM `{table}`')
    existing = cur2.fetchone()['cnt']
    ...
    print(f'{table}: {existing} already done, {len(rows)} remaining')
    
    # Write in batches of 100
    batch_size = 100
    for idx, i in enumerate(range(0, len(rows), batch_size)):
        batch = rows[i:i+batch_size]
        with open(f'{OUT}\\{table}_batch_{idx}.sql', 'w') as f:
            for row in batch:
                vals = []
                for c in cols:
                    v = row.get(c)
